...
def run_full_validation_pipeline(
    csv_content: str, eld_file=None, validation_run=None, save_to_db: bool = True
    ...
    if (
        not checksum_res["is_valid"]
        and checksum_res["provided_file_checksum"].upper()
        != checksum_res["file_checksum"].upper()
    ):
        checksum_failures_dicts.append(
            {
    ...
    if save_to_db and validation_run and checksum_failures_dicts:
        ...
        for cf in checksum_failures_dicts:
            checksum_failures_to_create.append(
                ValidationFailure(
                    validation_run=validation_run,
    ...
    from apps.validations.fmcsa_agents import (
        HeaderValidationAgent,
        UserValidationAgent,
        CMVValidationAgent,
        EventValidationAgent,
        LoginLogoutValidationAgent,
        EngineHoursValidationAgent,
    ...
    inv_results = inv_agent.investigate(
        failures_list, diag_results, malf_results, events_dict_list
    )

    if save_to_db and validation_run and inv_results:
        db_inv_results = []
        for inv in inv_results:
            db_inv_results.append(
                InvestigationResult(
                    validation_run=validation_run,
    ...
    severity = "NONE"
    if (
        len(hos_violations) > 0
        or len(checksum_res["failed_lines"]) > 0
        or fmcsa_failures_count > 0
        or len(diag_results) > 0
        or len(malf_results) > 0
    ):
        severity = "WARNING"
    if (
        len(hos_violations) > 2
        or not checksum_res["is_valid"]
        or fmcsa_failures_count > 3
        or len(diag_results) > 2
        or len(malf_results) > 0
    ):
        severity = "CRITICAL"
    ...
    metrics = {
        ...
        "is_valid": (
            len(hos_violations) == 0
            and fmcsa_failures_count == 0
            and len(checksum_res["failed_lines"]) == 0
            and checksum_res["is_valid"]
            and len(diag_results) == 0
            and len(malf_results) == 0
        ),
    }
