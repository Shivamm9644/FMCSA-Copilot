...
from apps.parser.models import ELDHeaderRecord, UserRecord, CMVRecord, ELDEventRecord
...
class BaseSegmentParser:
    ...
    @staticmethod
    def safe_decimal(
        val: Optional[str], default: Optional[Decimal] = None
    ) -> Optional[Decimal]:
        if not val:
            return default
        try:
        ...
        if row[0] == "ELD Event List":
            ...
            try:
                return ELDEventRecord(
                    ...
                    accumulated_engine_hours=self.safe_decimal(row[8])
                    if len(row) > 8
                    else None,
